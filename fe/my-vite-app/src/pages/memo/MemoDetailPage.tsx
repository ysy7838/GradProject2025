...
import {getMemo, deleteMemo, updateMemo, getRecommendedTags, getRecommendedMemos} from "@/services/memo";
    ...
    <div className="flex flex-col">
      <div className={tagContainerClasses}>
        {tagNames.map((tag, index) => (
          <span key={index} className="inline-flex items-center px-2 py-1 bg-gray-200 text-gray-800 text-sm rounded">
            {tag}
          </span>
        ))}
      </div>
      {tagNames.length > 5 && (
        <button onClick={() => setShowAll(!showAll)} className="text-primary text-sm mt-1 self-start">
          {showAll ? "간략히 보기" : "더보기"}
        </button>
      )}
    </div>
...
export default function MemoDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-white">
      <Header
        actions={["back", "edit", "more"]} // 👈 규칙에 맞는 버튼 설정
      ...
      <Modal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)}>
        <div className="p-6 text-center">
          ...
          <div className="flex justify-center space-x-4">
            <button onClick={handleConfirmDelete} className="px-4 py-2 bg-red-500 text-white rounded-lg">
              삭제
            </button>
            <button onClick={() => setShowDeleteModal(false)} className="px-4 py-2 bg-gray-200 rounded-lg">
              취소
            </button>
          </div>
        </div>
